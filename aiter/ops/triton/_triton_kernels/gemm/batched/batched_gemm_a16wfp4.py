...
def _batched_gemm_a16wfp4_kernel(
    ...
    if (pid_k * SPLITK_BLOCK_SIZE // 2) < K:
        ...
        # Block-local scale-group index, for masking the tail exactly the way
        # the a/b loads below are masked.
        ...
        for k in range(pid_k * num_k_iter, (pid_k + 1) * num_k_iter):
            if EVEN_K:
                ...
                # The scale operand has only cdiv(2*K, SCALE_GROUP_SIZE) groups
                # along K, but this block reads BLOCK_SIZE_K // SCALE_GROUP_SIZE
                # of them. Unmasked, the last (batch, n) tile runs off the end of
                # the scales allocation; if the bytes there happen to be 0xFF
                # (e8m0 NaN) the dot yields NaN even though the matching a/b
                # lanes are masked to zero, because 0 * NaN = NaN. That poisoned
                # out[:, B-1, N-1] for GLM-5.2 at K=192.
                b_scales = tl.load(
                    b_scale_ptrs,
            ...
            if EVEN_K:
                ...
                a_bf16 = tl.load(
                    a_ptrs, mask=offs_k_bf16[None, :] < 2 * K - k * BLOCK_SIZE_K, other=0
                )
                b = tl.load(
