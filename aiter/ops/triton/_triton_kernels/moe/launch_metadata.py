...
def matmul_launch_metadata(grid, kernel, args):
    """Shared launch-metadata hook for the a16w4 MoE GEMM kernels (triton + gluon):
    per-launch name + flop/byte counts (from the real args, routing-scaled) for
    the proton profiler, so both backends report identical flops/bytes."""
    ret = dict()
