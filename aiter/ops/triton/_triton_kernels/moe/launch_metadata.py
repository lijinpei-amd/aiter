...
def matmul_launch_metadata(grid, kernel, args):
    """Launch-metadata hook shared by the a16w4 MoE GEMM kernels (triton + gluon).

    Reports the per-launch name plus flop and byte counts (read by the proton
    profiler) from the actual kernel args, so both backends attribute identical
    flops/bytes for the same shape. Byte counts reflect the real routing: weight
    traffic is scaled by the number of active experts, and activation traffic by
    the gathered/scattered row counts.
    """
    ret = dict()
