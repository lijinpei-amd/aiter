...
def _preload_tile(
    ...
    if SWIZZLE_MX_SCALE == "GFX1250_SCALE":
        ...
        # along K. Scale layout derived by stripping the k_scale register-K identity
        # (DOT_LAYOUT_W == L_SCALE_W). Raw e8m0 byte replicated across a uint32 so
...
def _moe_gemm_a16w4_gluon_impl(
    ...
    # k_width=K_WIDTH. k_width doubles on unpack so the bf16 output matches
    # DOT_LAYOUT_W directly -> WMMA B operand needs NO convert_layout (saves 128
    # cross-lane v_permlanes/tile). Scale sits on the lanes the HW broadcast reads.
    ...
    # Blocked layouts for fp4-packed W and its expanded e8m0 scale; size_per_thread
    # along K doubles for the scale so unpack lines up element-for-element.
    ...
    # Parametric blocked layout for the compact scale tile (shape-agnostic, so
    # BLOCK_K stays tunable); the result is convert_layout'd to the upcast output
    # layout, so this layout only needs to tile the shape.
    COMPACT_SCALE_LAYOUT: gl.constexpr = gl.BlockedLayout(
        [1, 1], [8, 4], [num_warps, 1], [1, 0]
    )
    # scaled_upcast requires the expanded scale in the op's output layout (packed
    # input's blocked layout with sizePerThread doubled along the unpack axis).
    # Route through matched blocked layouts, then convert_layout to the WMMA layout.
