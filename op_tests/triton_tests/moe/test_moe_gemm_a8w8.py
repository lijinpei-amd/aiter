...
# ---------------------------------------------------------------------------
# A8W8. 'mxfloat8_e4m3fn' is MiniMax-M3's official MXFP8 format; plain
# 'float8_e4m3fn' stands in for the GLM-5.2-FP8 release. Both are run over every
# model shape -- the point is to exercise codegen at these (N,K,E).
...
@pytest.mark.parametrize("dtype_str", ["mxfloat8_e4m3fn", "float8_e4m3fn"])
@pytest.mark.parametrize("shape", _MODEL_SHAPES, ids=[s.id for s in _MODEL_SHAPES])
def test_model_shapes(shape, dtype_str, device="cuda"):
