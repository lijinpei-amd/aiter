...
def main(args: list[str] | None = None) -> None:
    ...
    if parsed_args.model is not None:
        if parsed_args.shape is not None or parsed_args.experts is not None:
            raise SystemExit("--model is mutually exclusive with --shape/--experts")
        dim1, dim2, total_experts, active_experts = _resolve_model_shape(
            parsed_args.model
        )
    else:
        if parsed_args.shape is None or parsed_args.experts is None:
