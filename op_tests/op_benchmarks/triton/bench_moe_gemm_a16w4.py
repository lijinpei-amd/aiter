...
def op_bytes_flops(batch, dim1, dim2, n_expts_act, n_active, gemm="both"):
    ...
    GEMM1 (gate/up), GEMM2 (down) or the full two-GEMM layer ("both").
...
def _measure(fn, timing, warmup, iters):
    ...
    (time-based) warmup/rep; the events path uses `warmup`/`iters` counts.
    """
    if timing == "do_bench":
...
# --- autotuning -----------------------------------------------------------
# Search space for --tune: the tuned config is the Cartesian product of these
# per-parameter value lists. block_m is fixed by routing (not tuned); backend and
# (optionally) the gluon stage are pinned by --backend / --gluon-stage. Edit these
# lists to widen/narrow the search (the product can be large -> tuning is slow).
_TUNE_SPACE = {
    "block_n": [64, 128, 256],
...
def _make_is_close(ref):
    """Build a predicate that checks a kernel output against `ref` using the unit
    test's assert_close tolerances (maxtol=4e-1, rmstol=4e-2). assert_close dumps
    the mismatch list on failure, so its stdout is suppressed here."""
    import contextlib
    import io
...
def _eval_config(make_fn, cfg, is_close, timing, warmup, iters):
    """Evaluate one config. Returns (min_ms, status) where status is:
    'ok' (correct), 'wrong' (ran but numerically incorrect vs the reference) or
    'fail' (did not compile/launch, e.g. an LDS-over-budget tile)."""
    try:
        out = make_fn(cfg)()
    except Exception:
        return None, "fail"
...
def tune_shape(
    ...
    """Autotune GEMM1 (gate/up) and/or GEMM2 (down) separately for one MLP shape.
    `gemm` selects which to tune ("both", "gemm1" or "gemm2")."""
    dev = "cuda:0"
    if seed is not None:
    ...
    # Tune against compact (non-swizzled) scales: the tile config is what's being
    # tuned, and compact scales are the common denominator both backends accept
    # (gluon rejects swizzled scales).
    sw1 = sw2 = None
...
def bench_mlp_single_weight_init(
    ...
    # GEMM1 (gate/up): gather + swiglu -> intermediate (M*topk, dim2/2). Its output
    # is stashed so GEMM2 can be benchmarked on the real intermediate.
    interm_holder = {}
    ...
    # GEMM2 (down): scatter-reduce with router weights, no swiglu -> (M, dim1). This
    # matches the production fused_experts down-projection the capture replay uses.
    def run_gemm2():
...
def parse_args(args: list[str] | None = None):
    ...
    parser.add_argument(
        ...
        "like the capture replay and needs no profiler; 'do_bench' uses triton's "
        "testing.do_bench (L2-flushing, time-based warmup/rep); 'proton' uses the "
        "triton proton profiler (requires librocprofiler-sdk). Tuning uses "
    ...
    parser.add_argument(
        "--tune",
        action="store_true",
        help="Autotune configs for each shape instead of benchmarking: sweep the "
        "Cartesian product of the config search space (see _TUNE_SPACE) for GEMM1 "
        "and GEMM2 of every M, and report the fastest configs. --backend / "
        "--gluon-stage pin the backend / stage that is tuned.",
    )
    parser.add_argument(
        "--tune-gap-percent",
        type=float,
        default=5.0,
        help="When tuning, only return configs whose kernel time is within this "
        "percent of the best config (default: 5).",
    )
    parser.add_argument(
        "--tune-cand-number",
        type=int,
    ...
    parser.add_argument(
        ...
        default="both",
        help="Which projection to benchmark/tune: 'gemm1' = gate/up only "
        "(gather + swiglu), 'gemm2' = down only (scatter, no swiglu), 'both' = the "
        "full two-GEMM layer (default). 'gemm1'/'gemm2' benchmark and tune the two "
